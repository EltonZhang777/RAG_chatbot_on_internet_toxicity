...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="internet toxicity RAG Assistant",
    page_icon="📖",
    layout="wide"
)
...
with st.sidebar:
    ...
    db_path = st.text_input(label = "Database Path", value = config.DEFAULT_DB_PATH)
    
    # Store in session state
    st.session_state.db_path = db_path
    
    # TO DO: Top K results with an interactive slider bar
    top_k = st.slider(label = "top K results", 
                      min_value = 1, max_value = 100, value = config.DEFAULT_TOP_K)
    # Store in session state
    st.session_state.top_k = top_k
    
    # TO DO: Model selection dropdown menu
    model_choice = st.selectbox(label = "Select Model", 
    ...
    max_iter = st.slider(label = "Max iterations", 
                         min_value = 10, max_value = 500, value = config.DEFAULT_MAX_ITER)
    ...
    if st.button("🗑️ Clear Chat History"):
        st.session_state.messages = []
        st.text("Successfully cleared chat history")
    ...
    st.markdown("""
    ### About
    This RAG assistant provides information and suggestions on internet toxicity based on:
    - internet toxicity analysis videos' transcripts
    - toxicity classification training sets
    - articles on the topic of internet toxicity
...
# Main App Header
# -----------------------------------------------------------------------------
st.title("📖 internet toxicity analysis RAG Assistant")
st.markdown("Ask questions about internet toxicity and get AI-powered answers based on a curated content database.")
...
with st.expander("💡 Example Questions"):
    examples = [
        "Give me an overview of the topic of internet toxicity.",
        "What platforms are most affected by internet toxicity?",
        "How are social media regulations addressing internet toxicity?",
        "How to protect oneself from internet toxicity using available resources on the social media platforms?",
        "How does Twitter's algorithm affect internet toxicity?"
    ]
