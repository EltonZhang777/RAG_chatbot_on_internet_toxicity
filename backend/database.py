...
class RAGDatabase:
    ...
    def __init__(self, db_path: str):
        ...
        # LOAD MODEL METHOD
        self.model = self._load_model(config.EMBEDDING_MODEL_NAME)

    # TO DO: CREATE _load_model() method that stores model attribute
    # @st.cache_resource because Streamlit reruns entire script on every interaction
    # @st.cache_resource stores information throughout session
    # @staticmethod caches info because functions can't use 'self'
    @staticmethod
    @st.cache_resource
    def _load_model(modelName: str) -> SentenceTransformer:
        try:
            model = SentenceTransformer(modelName)
            return model
        except Exception as e:
            raise Exception(f"Failed to load embedding model with Error `{str(e)}`!")
    ...
    def query(self, query_text: str, top_k: int = 10) -> list[dict]:
        ...
        try:
            ...
            results = conn.execute(f"""
                SELECT text, array_cosine_similarity(embedding, ?::FLOAT[{config.EMBEDDING_DIMENSION}]) as similarity
                FROM rag_documents
            ...
            formatted_results = [{"text": item[0], "similarity": item[1]} for item in results]
